...
volatile int highCyc = 0, lowCyc = 0, highCyc2 = 0, lowCyc2 = 0;
void __attribute__((__interrupt__, __auto_psv__)) _INT0Interrupt(void){
    ...
    if (numFall == 1 && numRise == 1){
        highCyc = TMR1; //cycles 1000-2000
        TMR1 = 0;
    }
    if (numRise == 2 && numFall == 1){
        lowCyc = TMR1; //cycles 2000-2500
        TMR1 = 0;
    }
    if (numFall == 2 && numRise == 2){
        highCyc2 = TMR1; //cycles 2500-3000
        TMR1 = 0;
    }
    if (numRise == 3 && numFall == 2){
        lowCyc2 = TMR1; //cycles 3000-4000
        TMR1 = 0;
    }
    _INT0EP = 1 - _INT0EP;
}
